...
- 2026-05-19 | bootstrap_pnpm_runner | Se añadió `scripts/pnpmw.sh`, `init.sh` ahora acepta `corepack pnpm` y el repo documenta cómo instalar `pnpm` con `npm` o `yarn` si falta en PATH. | `PATH="/private/tmp:$PATH" ./init.sh`. | Próximo frente: `safe_write_file` con flujo SDD completo.
