...
harness es un servidor MCP local por `stdio`.
...
- `src/logger.ts` escribe logs JSONL en `.harness/logs`
...
- La config no debe depender del cwd del cliente; usar `--config` o `harness_CONFIG` cuando aplique
